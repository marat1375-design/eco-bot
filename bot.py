...
SYSTEM_PROMPT = (
    ...
    "НАРУШЕНИЕ:\n"
    "[Одним предложением опиши, что выявлено. Например: Выявлена замазученность территории вокруг контейнерной площадки для накопления отходов, что создает риск загрязнения почвы нефтепродуктами.]\n\n"
    ...
    "- Обеспечить содержание контейнерной площадки в надлежащем санитарном и экологическом состоянии.\n"
    "- При необходимости выполнить фотофиксацию после устранения и представить подтверждение в ООС.\n\n"

    "ПРИ ПРОВЕРКЕ ГОСОРГАНАМИ:\n"
    "[Укажи только конкретную статью КоАП, если она есть в предоставленных фрагментах. Напиши размер штрафа только если он прямо указан в предоставленном фрагменте. Если подходящей статьи КоАП нет, напиши: В предоставленных фрагментах подходящая статья КоАП не найдена.]\n\n"

    "ВАЖНО:\n"
    "- Не используй статьи 3 и 4 Экологического кодекса как основное нарушение.\n"
    "- Не указывай штраф в тенге, если размер МРП или расчет не предоставлены в контексте.\n"
...
def normalize_query(query):
    ...
    replacements = {
        "мусорки": "мусорный контейнер контейнерная площадка место накопления отходов",
        "мусорка": "мусорный контейнер контейнерная площадка место накопления отходов",
        ...
        "тбо": "коммунальные отходы отходы место накопления отходов контейнерная площадка",
    }
...
def score_article(block, query, law_key):
    ...
    if any(x in query_expanded for x in ["мусор", "контейнер", "отход", "тбо", "коммунальн"]):
        for term in ["отход", "накоплен", "сбор", "размещен", "захоронен", "коммунальн"]:
            if term in block_lower:
                score += 6
...
print("БОТ ЗАПУЩЕН - adilet.zan.kz + Gemini + pyTelegramBotAPI")
