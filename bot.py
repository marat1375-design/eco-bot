...
Нарушение экологических требований к операциям по управлению отходами может повлечь административную ответственность.
Возможная квалификация применяется госорганом с учетом фактических обстоятельств.
...
SYSTEM_PROMPT = (
    ...
    "- Не используй статью 397 Экологического кодекса, если запрос не связан с бурением, скважинами, добычей, недропользованием или проектными документами.\n- Не используй статью 350 Экологического кодекса для обычного контейнера или места накопления отходов, если вопрос не связан с полигоном, захоронением или размещением отходов на полигоне.\n"
    ...
    "- Не используй Markdown-разметку: не ставь звездочки, таблицы, решетки и лишнее форматирование. Не удаляй символ # из ссылок Adilet.\n"
    ...
    "[Укажи подходящие статьи КоАП РК только если они есть в предоставленном контексте. Формулируй осторожно: может быть квалифицировано. Обязательно указывай ссылку, если она есть в контексте. Если в контексте есть статья 344 по отходам или статья 324 по загрязнению окружающей среды, используй их в этом разделе по смыслу. Если подходящих статей КоАП нет, напиши: В предоставленных фрагментах подходящая статья КоАП не найдена.]\n"
)
...
PHOTO_PROMPT = (
    ...
    "Не используй Markdown-разметку, звездочки, списки со звездочками и таблицы. Пиши обычным текстом.\n"
    "Не перечисляй слишком мелкие предметы с полной уверенностью. Если видны смешанные отходы, пиши осторожно: визуально имеются смешанные коммунальные отходы, включая пищевые остатки и упаковочные материалы.\n"
...
def is_bad_article(law_key, block, query):
    ...
    if law_key == "ecocode" and number == "350":
        text = normalize_query(query).lower()
        polygon_words = [
            "полигон", "захорон", "захоронение", "размещение на полигоне",
            "перед захоронением", "полигоне"
        ]
        if not any(word in text for word in polygon_words):
            return True
...
def get_forced_koap_context(query):
    ...
    environment_words = [
        "замазуч", "нефт", "масло", "мазут", "нефтепродукт", "разлив",
        "розлив", "утеч", "загряз", "почв", "земл", "грунт", "пластов",
        "сточн", "сброс", "перелив"
    ]

    waste_words = [
        "отход", "мусор", "контейнер", "тбо", "коммунальн", "раздельн",
        "сортиров", "накоплен", "хранен", "ртут", "ламп", "батар",
        "аккумулятор", "электрон", "ветош", "сорбент", "нефтешлам",
        "шлам", "буров"
    ]

    if any(word in text for word in environment_words):
...
def build_context(query):
    ...
    max_length = 18000
    available_for_laws = max_length - len(koap_context)
    if available_for_laws < 6000:
        available_for_laws = 6000

    return (koap_context + laws_context[:available_for_laws])[:max_length]
...
print("БОТ ЗАПУЩЕН - Эко Помощник ПККР 1.1 - исправлены ссылки, КоАП и статья 350")
