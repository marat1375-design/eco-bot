...
SYSTEM_PROMPT = (
    ...
    "[Обязательно проверь, есть ли в контексте статьи КоАП РК 324 или 344. Если они есть и подходят по смыслу, укажи их.]\n"
    ...
    "[Если размер штрафа указан в МРП, напиши размер в МРП.]\n"
    "[В тенге указывай расчет только для 2026 года из расчета 1 МРП = 4325 тенге.]\n"
    "[Если подходят обе статьи, укажи обе кратко.]\n"
    "[Пиши осторожно: может быть квалифицировано, возможна административная ответственность, окончательная квалификация относится к компетенции госоргана.]\n\n"
...
def build_context(query):
    ...
    if forced_koap:
        context += "\n=== КоАП РК от 05.07.2014 N 235-V - принудительно подобранные статьи для раздела ответственности ===\n"
        context += forced_koap
...
print("БОТ ЗАПУЩЕН - adilet.zan.kz + Gemini + pyTelegramBotAPI + КоАП")
